...
title: jiti 原理学习
author: Madinah
description: jiti 是一个直接用来运行 TS 和 ESM 的 node 运行时。本文记录了 napi-rs 的搭建过程，并添加了调试命令以增强调试体验。napi-rs 提供了方便的命令来让我们基于 Rust 开发 node 的 addon，farm 也是基于 napi-rs 开发的。文章还介绍了 jiti 的基本原理和使用方法，并列举了 jiti 的缺点。
tags:
  - nodejs
