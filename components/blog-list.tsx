...
export default function BlogsList() {
  ...
  const BlogListHeader = () => <div>
    <ul className="flex gap-4 mb-4 list-none font-mono pl-0">
      <li className={`cursor-pointer hover:opacity-100 text-black dark:text-white ${currentCategory === 'blog' ? 'opacity-100' : "opacity-50"} `} onClick={() => setCurrentCategory("blog")}>
        <span className="text-3xl">Blog</span>
      </li>
      <li className={`cursor-pointer hover:opacity-100 deep-hover text-black dark:text-white ${currentCategory === 'record' ? 'opacity-100' : "opacity-50"}`} onClick={() => setCurrentCategory('record')}>
        <span className="text-3xl">Record</span>
      </li>
    </ul>
    <div className="flex items-center my-2">
      <div className="flex-auto border-b border-dashed border-current opacity-30" />
      <div className="mx-4"> A total of <b>{length}</b> articles </div>
      <div className="flex-auto border-b border-dashed border-current opacity-30" />
    </div>
  </div>
      ...
      <div className="mt-[8em]">
        {
          sortedYears.map(key => {
            ...
            blogList = blogList.filter(blog => blog.category === currentCategory)
              ...
              <Text text={`${key}`} className="pointer-events-none !absolute -top-[0.8em]" />
              {
                blogList.map(({ name, title, readingTime, createTime }) => <a href={`/blog/${name}`} key={name}
                  ...
                  <li className="no-underline list-none flex gap-2 items-center font-mono">
                    <span className="text-sm opacity-70 font-mono">{new Date(createTime).toLocaleDateString()}</span>
